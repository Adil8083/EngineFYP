import React from 'react';
import {useState} from 'react';
import {
  Text,
  View,
  Button,
  Image,
  StyleSheet,
  ScrollView,
  TouchableOpacity,
  Dimensions,
} from 'react-native';
import MaterialIcons from 'react-native-vector-icons/MaterialIcons';
import DropShadow from 'react-native-drop-shadow';

import AppView from '../components/common/AppView';
import {SCREENS} from '../constants/SCREENS';
import Colors from '../Theme/colors';
import SocialAccButton from '../components/common/SocialAccButton';

const Height = Dimensions.get('window').height;
const Profile = ({navigation}) => {
  const [profilePic, setProfilePic] = useState();
  const [Name, setName] = useState('Keanu Reeves');
  const [Country, steCountry] = useState('Canada');
  const [Mail, setMail] = useState('keanu12reeves@gmail.com');
  const [achievements, setAchievements] = useState([
    {identifier: 1, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 2, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 3, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 4, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 5, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 6, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 7, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 8, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 9, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 10, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 11, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 12, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 13, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 14, name: 'abcde', description: 'abc', year: '2016'},
    {identifier: 15, name: 'abcde', description: 'abc', year: '2016'},
  ]);
  const [fbPath, setFbPath] = useState();
  const [InstaPath, setInstaPath] = useState();
  const [TwitterPath, setTwitterPath] = useState();
  const [YoutubePath, setYoutubePath] = useState();
  const [OpenAchievements, setOpenAchievements] = useState(false);
  return (
    <AppView>
      <View style={{marginTop: 10}}>
        <View style={styles.BioCont}>
          <View style={styles.ImgCont}>
            <DropShadow
              style={{
                shadowColor: 'black',
                shadowOffset: {
                  width: 0,
                  height: 0,
                },
                shadowOpacity: 0.67,
                shadowRadius: 5,
              }}>
              <Image
                source={{
                  uri:
                    'data:image/jpeg;base64,',
                }}
                style={styles.profileImg}
              />
            </DropShadow>
          </View>
          <DropShadow
            style={{
              shadowColor: 'black',
              shadowOffset: {
                width: 0,
                height: 0,
              },
              shadowOpacity: 0.67,
              shadowRadius: 5,
            }}>
            <Text style={styles.Name}>{Name}</Text>
          </DropShadow>
          <DropShadow
            style={{
              shadowColor: 'black',
              shadowOffset: {
                width: 0,
                height: 0,
              },
              shadowOpacity: 0.67,
              shadowRadius: 5,
            }}>
            <Text style={styles.Country}>{Country}</Text>
          </DropShadow>
          <Text style={styles.Mail}>{Mail}</Text>
        </View>

        <View style={[styles.AchvCont, {height: OpenAchievements ? 200 : 45}]}>
          <TouchableOpacity
            style={{
              flexDirection: 'row',
              justifyContent: 'space-between',
            }}
            onPress={() => setOpenAchievements(!OpenAchievements)}>
            <Text style={[styles.Name, {marginTop: 8.6}]}>Achievements</Text>
            {!OpenAchievements && (
              <MaterialIcons
                name="keyboard-arrow-down"
                size={24}
                color={Colors.TextColor}
                style={{marginTop: 8.6}}
              />
            )}
            {OpenAchievements && (
              <MaterialIcons
                name="keyboard-arrow-up"
                size={24}
                color={Colors.TextColor}
                style={{marginTop: 8.6}}
              />
            )}
          </TouchableOpacity>
          <ScrollView>
            {OpenAchievements &&
              achievements.map((obj) => (
                <View key={obj.identifier} style={{flexDirection: 'row'}}>
                  <Text style={styles.AchvInfo}>{obj.name}</Text>
                  <Text style={styles.AchvInfo}>{obj.year}</Text>
                  <Text
                    numberOfLines={3}
                    style={[styles.AchvInfo, {width: '63%', marginRight: 8}]}>
                    {obj.description}
                  </Text>
                </View>
              ))}
          </ScrollView>
        </View>
        <View style={styles.SocialInfoCont}>
          <SocialAccButton
            name="facebook"
            size={30}
            onPress={() => console.log('Facebook is pressed')}
            colorArr={['#2564E6', '#2B4AC3', '#2B4ECD']}
          />
          <SocialAccButton
            name="instagram"
            size={22}
            end={{x: 1, y: 0.6}}
            start={{x: 0, y: 0.8}}
            locations={[0.3, 0.8, 0.7]}
            onPress={() => console.log('instagram is pressed')}
            styleButton={{marginLeft: 15}}
            colorArr={['#AF34CC', '#FD7863', '#FC865E']}
          />
          <SocialAccButton
            name="twitter"
            size={22}
            end={{x: 0.8, y: 0}}
            start={{x: 0, y: 0.5}}
            locations={[0.3, 0.65, 0.7]}
            onPress={() => console.log('twitter is pressed')}
            styleButton={{marginLeft: 15}}
            colorArr={['#018AFF', '#02A5FD', '#05B0FE']}
          />
          <SocialAccButton
            name="youtube-play"
            size={24}
            end={{x: 0.8, y: 0}}
            start={{x: 0, y: 0.5}}
            locations={[0.1, 0.9, 0.2]}
            onPress={() => console.log('youtube is pressed')}
            styleButton={{marginLeft: 15}}
            colorArr={['#BA0100', '#EA0204', '#EA0204']}
          />
        </View>
      </View>
    </AppView>
  );
};
const styles = StyleSheet.create({
  profileImg: {
    width: '100%',
    height: '100%',
    borderRadius: 60,
  },
  BioCont: {
    alignItems: 'center',
  },
  ImgCont: {
    width: 120,
    height: 120,
  },
  Name: {
    marginTop: 6,
    fontWeight: 'bold',
    fontSize: 20,
    color: Colors.TextColor,
  },
  Country: {
    marginTop: 6,
    fontWeight: 'bold',
    fontSize: 16,
    color: Colors.TextColor,
  },
  Mail: {
    marginTop: 6,
    fontSize: 13,
    color: Colors.TextColor,
    textDecorationLine: 'underline',
  },
  AchvCont: {
    marginLeft: 10,
    marginTop: 30,
    backgroundColor: Colors.secondary,
    paddingHorizontal: 15,
    borderRadius: 10,
    marginBottom: 100,
  },
  AchvInfo: {
    color: Colors.TextColor,
    width: '20%',
    fontSize: 15,
    marginTop: 10,
    marginRight: 15,
  },
  SocialInfoCont: {
    flexDirection: 'row',
    alignSelf: 'center',
    position: 'absolute',
    top: Height - 150,
  },
});
export default Profile;
